...
object StreamingExampleWithHadoop {
    ...
    def main(args: Array[String]) {
        val conf = new SparkConf().setAppName("StreamingExampleWithHadoop").set("spark.cores.max", "4")
        val ssc = new StreamingContext(conf, Seconds(60))
        val fs = FileSystem.get(ssc.sparkContext.hadoopConfiguration)

        val newUserRelationLines = ssc.socketTextStream("kenai", args(0).toInt)
        val newUserHashtagLines = ssc.socketTextStream("kenai", args(1).toInt)
        val newUserDataLines = ssc.socketTextStream("kenai", args(2).toInt)
